...
export class MoneyTypeService extends SearchService<MoneyType> {
  ...
  searchPage(options: PagedGetOption | undefined, queryName: string):
    ...
    return this.hateoasService.searchPage<MoneyType>(MoneyType, queryName, options);
  }

  getPage(options: PagedGetOption | undefined): Observable<PagedResourceCollection<MoneyType>> {
    return this.hateoasService.getPage<MoneyType>(MoneyType, options);
  }

  getCurrencyByCode(code: string): Observable<MoneyType> {
    return this.hateoasService.searchResource<MoneyType>(MoneyType, "findByCode", {
        params: {
          code: code
        }
    })
