...
export class OrganizationGroupsService extends SearchService<OrganizationGroup> {
  ...
  searchPage(options: PagedGetOption | undefined, queryName: string):
    ...
    return this.hateoasService.searchPage<OrganizationGroup>(OrganizationGroup, queryName, options);
  }

  getPage(options: PagedGetOption | undefined): Observable<PagedResourceCollection<OrganizationGroup>> {
    return this.hateoasService.getPage<OrganizationGroup>(OrganizationGroup, options);
  }
  ...
  getPathForOrganizationGroup(organizationGroupId: string | null): Observable<ResourceCollection<OrganizationGroup>> {
    ...
    if (organizationGroupId) {
      return this.hateoasService.searchCollection<OrganizationGroup>(OrganizationGroup, 'pathById', {
        params: {
